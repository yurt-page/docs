...
As an alternative to it was created an open source Jabber protocol, but it's failed by all means.
...
I call them "mass messengers" and today everybody forgot the old Instant Messengers (IM).
...
2. Almost unlimited cloud storage for photos, videos and voice messages. Many people just using messengers to transfer files. With XMPP servers that was almost not possible. 
3. Using just a phone number as your id instead of ICQ number or an XMPP `nickname@server`. This is reusing of an existing centralization.
...
5. Just don't use encryption and use a binary protocols for the best speed. XMPP folks spent a lot of time on this topic and mostly failed.
...
An adoption of mass messengers was so fast that in almost all countries almost all users "belongs" to a just a few silos.
...
People just didn't have enough of understanding of all danger. And they won't have it.
We have infinitive data leak: even if not from you then from your wife or coworkers.
A great news here is that messenger companies tried hard to at least stop botnets.

Interesting that even if some government decided to protect (or expose) citizens and block a messenger this was already too painful.
People consider their messengers as their private communication and react to blocking extremely negatively.
...
With a mass messengers you never had such a problem: you have only one client for a Viber protocol i.e. the Viber itself.
...
But it also can be useful in a natural way. For example, I'm using different messengers for different needs:
...
* [Meshenger](https://github.com/meshenger-app/meshenger-android) P2P calls without the need for accounts or access to the Internet. There is no discovery mechanism, no meshing and no servers: just scan each others QR-Code. Android only. Works badly. Its author is known adequate man who developed KadNode. So even if now this may be not that good choose it may be mush easier to make the Meshenger as a good product. 
* Jami - a DHT based and somewhat can work locally if install their OpenDHT server. Looks more or less adequate. 
* Briar - mash messenger over Bluetooth. It was created for coordination of mass protests and almost not usable for regular users. 
...
* Own Matrix server: written in Python and too heavy, now there are attempts to write a server in Rust. The main advantage that it has a well funded company behind and has good clients e.g. Element.io. Still, it's not that much better than XMPP which is developed in more open manner by FOSS enthusiasts. Today this is a default option for a user-friendly secure messengers but in fact this is just XMPP clients are less developed. It would be better to invest into XMPP ecosystem to get the same level of quality but more sustainable.
...
* https://conversejs.org/ an XMPP client in browser and with Electron on desktop and mobile (TBD). This is a very nice idea. JS will eat world.

The best comparison of secure chats https://bkil.gitlab.io/secuchart/
