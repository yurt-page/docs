...
For many regular users it may be too complicated to install a web server to run a yurt on their desktop computer. But this is an extreemely important to support because desktop PC has a lot of storage and a really good CPU. This will be really needed for photos backup.
...
2. Provide an installer that will setup the Yurt with one of the existing web server.
3. Create a Yurt as a separate programm with an embedded webserver. This will give more independence and an ability to move faster but this also means that Yurt must always provide updates especally with security fixes.

We must start with the first option as a more easier to begin with. Most target users of Yurt using Windows, MacOS is too expensive for most peoples and Linux users... they know better how to install and use Yurt.
...
* Tomcat: needs Java, slow, but (potentially) safier. Maybe we may create some GUI based on Jetty.
* [BusyBox W32](https://frippery.org/busybox/) may be good solution for us but it doesn't have any GUI.
* [Miranda IM + HTTPServer plugin](https://github.com/miranda-ng/miranda-ng/tree/master/plugins/HTTPServer) surpisingly but this is one of the most easy to use solution. It even supports UPNP port forwarding (but no NAT-PMP/PCP). It even has some default website template that allows to see dirrectory listing. It doesn't support CGI and needs to be reviseted and improved. One good thing about it is that many users remember Miranda. It is also very lightweiht and can be used to setup a Jabber/XMPP account.
* [200 OK  Server Chrome Plugin](https://chrome.google.com/webstore/detail/web-server-for-chrome/ofhbbkphhbklhfoeikjpcbhemlocgigb) a simple webserver that is very easy to install for unexpirienced users. No CGI but has UPNP port forwarding (but no NAT-PMP/PCP). Maybe this is the only option for Chromebook.
